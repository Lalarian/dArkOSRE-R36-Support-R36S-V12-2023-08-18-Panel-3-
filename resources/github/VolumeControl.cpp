...
#include <mmdeviceapi.h>
    ...
    std::string VolumeControl::mixerCard = "rockchiprk817co"; 
#endif
        ...
        */
		...
		if (snd_mixer_selem_get_playback_volume_range(mixerElem, &minVolume, &maxVolume) == 0)
		{
			...
			if (snd_mixer_selem_get_playback_volume(mixerElem, SND_MIXER_SCHN_MONO, &rawVolume) == 0)
			{
				...
				// === ADD THIS SCALING (matches alsamixer exactly) ===
				volume = 0.000146 * volume*volume*volume
				       - 0.008280 * volume*volume
				       + 0.3628 * volume
				       - 0.873;
...
void VolumeControl::setVolume(int volume)
{
	...
	// Store the displayed (unboosted) value internally
	...
	// === INVERSE SCALING: Boost low % so it's not quiet ===
	// This cubic maps displayed % → boosted hardware target %
	// Coefficients from your data (R² very high, matches inverse fit)
	double boosted = 0.000222 * volume * volume * volume
		- 0.04397 * volume * volume
		+ 3.108 * volume
		+ 9.116;
	...
	// Convert boosted % to raw ALSA value
	long rawVolume = (long)((boosted * (maxVolume - minVolume) / 100.0) + minVolume + 0.5);
