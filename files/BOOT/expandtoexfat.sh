...
(
	...
	cd /tempthemes
	(tar --no-same-permissions --no-same-owner --warning=no-timestamp -cf - . | pv -n -s $(du -sb /tempthemes/ | awk '{print $1}') | tar --warning=no-timestamp --no-same-permissions --no-same-permissions --no-same-owner -xf - -C /roms/themes/) &> /tmp/tar_stat.txt &
	let COUNT=1
	while true; do
	  VAL=$(tail -n 1 /tmp/tar_stat.txt)
	  if [[ "$VAL" -ge "100" ]]; then
